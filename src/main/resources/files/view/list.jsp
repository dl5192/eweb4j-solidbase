		...
		<c:forEach var="dataItem" items="${listPage.trdatas}">
			<tr target="pojo_id" rel="${dataItem.id}">
				...
				<c:forEach var="item" items="${dataItem.datas }">
					<td onclick="selectBox('${random}${dataItem.id }')" name="ids">
					<c:if test="${('image' == dataItem.cate.name)}">
				...
				<td>
					...
					<a title="编辑files_${dataItem.id}" target="dialog" href="${BaseURL}${listPage.model}/${dataItem.id}/edit" rel="${listPage.model}_${dataItem.id}_edit" class="btnEdit">编辑</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
